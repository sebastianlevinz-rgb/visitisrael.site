/**
 * Tel Aviv canonical (EN + HE) — content-level invariants.
 *
 * Phase 3 plan 01 Task 2: validates MDX content authored in
 * `content/{en,he}/regions/tel-aviv.mdx` after Velite compilation.
 *
 * Mirrors `jerusalem-en-canonical.test.ts` + `jerusalem-he-canonical.test.ts`
 * shape, parameterized for Tel Aviv. Tasks 2 and 3 (sub-destinations) share
 * this file; the canonical assertions live here and sub-dest assertions are
 * appended by Task 3.
 */
...
function findEn(): RegionEntry | undefined {
  return loadRegions().find((r) => r.lang === 'en' && r.region === 'tel-aviv');
}

function findHe(): RegionEntry | undefined {
  return loadRegions().find((r) => r.lang === 'he' && r.region === 'tel-aviv');
}

const EN_MDX_PATH = resolve(process.cwd(), 'content/en/regions/tel-aviv.mdx');
const HE_MDX_PATH = resolve(process.cwd(), 'content/he/regions/tel-aviv.mdx');
...
describe('Tel Aviv EN canonical (Velite Region)', () => {
  ...
  it('compiled body contains the primary keyword "Tel Aviv" in prose', () => {
    const r = findEn();
    if (!r) return;
    const occurrences = r.body.match(/Tel\s*Aviv/gi);
...
describe('Tel Aviv EN MDX (raw source)', () => {
...
describe('Tel Aviv HE canonical (Velite Region)', () => {
  it.skipIf(!HAS_HE_MDX)('exists in the Velite regions collection', () => {
    const r = findHe();
    expect(r, 'Run `pnpm velite` first to compile MDX').toBeDefined();
  });

  it('has frontmatter slug=tel-aviv + region=tel-aviv', () => {
    const r = findHe();
    if (!r) return;
    expect(r.slug).toBe('tel-aviv');
    expect(r.region).toBe('tel-aviv');
  });

  it('compiled body contains primary HE keyword מה לעשות בתל אביב or equivalent variant', () => {
    const r = findHe();
    if (!r) return;
    // Accept any of: "מה לעשות בתל אביב" / "תל אביב" mention density >=5
    const telAvivMentions = r.body.match(/תל\s*אביב/g);
    expect(telAvivMentions, 'primary HE entity missing').not.toBeNull();
    expect((telAvivMentions as RegExpMatchArray).length).toBeGreaterThanOrEqual(
      5,
    );
  });
  ...
  it('Latin brand names wrapped in <span dir="ltr"> (AUD-024)', () => {
    ...
    // If Booking.com or TLV appear, they MUST be inside a dir="ltr" span.
...
describe('Tel Aviv HE MDX (raw source) — word-count parity', () => {
...
// ============================================================
// Tel Aviv sub-destinations (Task 3 — same file, appended by plan)
// ============================================================

const TEL_AVIV_SUB_DEST_SLUGS = [
  'tel-aviv-old-jaffa',
...
describe('Tel Aviv sub-destinations (7 EN + 7 HE pairs)', () => {
  for (const slug of TEL_AVIV_SUB_DEST_SLUGS) {
    ...
    describe(slug, () => {
      ...
      it.skipIf(!hasEn)('EN frontmatter has parentRegion: tel-aviv', () => {
        const raw = readFileSync(enPath, 'utf8');
        expect(/parentRegion:\s*tel-aviv/.test(raw)).toBe(true);
      });

      it.skipIf(!hasHe)('HE frontmatter has parentRegion: tel-aviv', () => {
      ...
      it.skipIf(!hasEn)(
        'EN does NOT carry religiousSiteId (TLV sub-dests are TouristAttraction only)',
        () => {
          const raw = readFileSync(enPath, 'utf8');
