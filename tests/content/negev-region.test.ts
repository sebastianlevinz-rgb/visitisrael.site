/**
 * Negev Desert canonical (EN + HE) + 5 sub-destination pair invariants.
 *
 * Phase 3 plan 05 Task 2 + Task 3: validates MDX content authored in
 * `content/{en,he}/regions/negev.mdx` plus
 * `content/{en,he}/sub-destinations/negev-*.mdx` after Velite compilation.
 *
 * Mirrors `dead-sea-region.test.ts` shape, parameterized for Negev.
 *
 * Special invariants for Negev (per plan 05 critical_context):
 *   - REG-05 image-gap canary: data/negev-images.md exists with $1,500 substring
 *   - Bedouin hospitality framing respectful (no "exotic" / "tribal" tone)
 *   - Negev has NO PlaceOfWorship sub-dests (Avdat archaeological + Ben-Gurion
 *     grave national-memorial; all TouristAttraction-only)
 *   - No religiousSiteId frontmatter on any sub-dest
 *   - HE primary keyword 'מה לעשות בנגב'
 */
...
function findEn(): RegionEntry | undefined {
  return loadRegions().find((r) => r.lang === 'en' && r.region === 'negev');
}

function findHe(): RegionEntry | undefined {
  return loadRegions().find((r) => r.lang === 'he' && r.region === 'negev');
}

const EN_MDX_PATH = resolve(process.cwd(), 'content/en/regions/negev.mdx');
const HE_MDX_PATH = resolve(process.cwd(), 'content/he/regions/negev.mdx');
...
describe('Negev EN canonical (Velite Region)', () => {
  ...
  it('compiled body contains the primary keyword "Negev" in prose', () => {
    const r = findEn();
    if (!r) return;
    const occurrences = r.body.match(/Negev/g);
  ...
  it('never uses "exotic" framing for Bedouin (respectful framing lock)', () => {
    if (!HAS_EN_MDX) return;
    const raw = readFileSync(EN_MDX_PATH, 'utf8');
    expect(/\bexotic\b/i.test(raw)).toBe(false);
    expect(/native\s+experience/i.test(raw)).toBe(false);
  });
...
describe('Negev EN MDX (raw source)', () => {
...
describe('Negev HE canonical (Velite Region)', () => {
  it.skipIf(!HAS_HE_MDX)('exists in the Velite regions collection', () => {
    const r = findHe();
    expect(r, 'Run `pnpm velite` first to compile MDX').toBeDefined();
  });

  it('has frontmatter slug=negev + region=negev', () => {
    const r = findHe();
    if (!r) return;
    expect(r.slug).toBe('negev');
    expect(r.region).toBe('negev');
  });

  it('compiled body contains primary HE keyword נגב', () => {
    const r = findHe();
    if (!r) return;
    const mentions = r.body.match(/נגב/g);
...
describe('Negev HE MDX (raw source) — word-count parity', () => {
...
// ============================================================
// REG-05 image-gap canary deliverable
// ============================================================

describe('Negev REG-05 image-gap policy deliverable', () => {
  it('data/negev-images.md exists', () => {
    expect(existsSync(resolve(process.cwd(), 'data/negev-images.md'))).toBe(
      true,
    );
  });

  it('data/negev-images.md contains "$1,500" Phase 6 budget substring', () => {
    const path = resolve(process.cwd(), 'data/negev-images.md');
    if (!existsSync(path)) return;
    const content = readFileSync(path, 'utf8');
    expect(content).toContain('$1,500');
  });

  it('data/negev-images.md documents Wikimedia coverage estimate', () => {
    const path = resolve(process.cwd(), 'data/negev-images.md');
    if (!existsSync(path)) return;
    const content = readFileSync(path, 'utf8');
    expect(/40-50%|Wikimedia/i.test(content)).toBe(true);
  });
});

// ============================================================
// Negev sub-destinations (Task 3 — 5 EN + 5 HE pairs)
// ============================================================

const NEGEV_SUB_DEST_SLUGS = [
  'negev-mitzpe-ramon',
  'negev-avdat',
  'negev-sde-boker',
  'negev-ein-avdat',
...
describe('Negev sub-destinations (5 EN + 5 HE pairs)', () => {
  for (const slug of NEGEV_SUB_DEST_SLUGS) {
    ...
    describe(slug, () => {
      ...
      it.skipIf(!hasEn)('EN frontmatter has parentRegion: negev', () => {
        const raw = readFileSync(enPath, 'utf8');
        expect(/parentRegion:\s*negev/.test(raw)).toBe(true);
      });

      it.skipIf(!hasHe)('HE frontmatter has parentRegion: negev', () => {
      ...
      it.skipIf(!hasEn)(
        'EN does NOT carry religiousSiteId (Avdat archaeological; Ben-Gurion grave national-memorial; Bedouin hospitality experiential)',
        () => {
          const raw = readFileSync(enPath, 'utf8');
      ...
      it.skipIf(!hasEn)(
        'EN does NOT use "exotic" framing for Bedouin (respectful framing lock)',
        () => {
          const raw = readFileSync(enPath, 'utf8');
          expect(/\bexotic\b/i.test(raw)).toBe(false);
        },
