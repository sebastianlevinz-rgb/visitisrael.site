...
must_haves:
  truths:
    ...
    - "`<RegionHero>` uses `priority` + `fetchpriority=\"high\"` (IMG-04 contract — first applied in Phase 2)"
...
Purpose: Establish the component contract — every Phase 2+ page composes from these. No content page can introduce raw hex, physical directional utilities, or untyped variants because they consume CVA + tokens via this library.
...
// from app/globals.css (plan 02) — tokens via Tailwind @theme
...
// components/travel/AffiliateCard.tsx (STUB — plan 06 wires helpers)
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test: All primitives use ONLY logical Tailwind utilities (verified by grep — no `ml-`, `pr-`, `text-left`, etc.)
...
Create all 7 primitives per RESEARCH.md §1.3 "Concrete steps — CVA example":

1. **`Button.tsx`** — verbatim from RESEARCH §1.3 (CVA: variant=primary|secondary|ghost, size=sm|md|lg; uses `bg-[var(--button-bg-primary)]` etc.). All references via `var(--...)` — NOT raw hex.
2. **`Card.tsx`** — CVA: variant=default|elevated|interactive; uses `bg-[var(--card-bg)]` + `border-[var(--card-border)]`
3. **`Tag.tsx`** — small pill with variant=neutral|success|warning|danger using semantic color tokens
4. **`Badge.tsx`** — similar to Tag but smaller/numeric
5. **`Section.tsx`** — semantic `<section>` with `as?: 'section'|'article'|'div'`, `padded?: boolean`
6. **`Container.tsx`** — max-width wrapper + logical horizontal padding (`ps-4 pe-4` becoming `mx-auto px-4` is OK since `px-` is logical-equivalent in Tailwind v4)
7. **`Grid.tsx`** — accepts `cols` + responsive breakpoints; uses `grid-cols-*` Tailwind utilities
...
<task type="auto" tdd="true">
  ...
  <behavior>
    - Test: `<SkipNav>` on HE locale renders Hebrew text "דלג לתוכן הראשי"
    ...
    - Test: `<AffiliateCard>` STUB renders `<AffiliateDisclosure>` BEFORE the (currently placeholder) affiliate `<a>` — DOM order asserted via `compareDocumentPosition`
  ...
  <action>
Per RESEARCH.md §1.3 and `hebrew-rtl-best-practices/SKILL.md`:

Build all 12 travel composites + 6 layout components:

**Travel composites** (in `components/travel/`):
1. **`RegionHero.tsx`** — props: `creditId`, `title`, `subtitle?`, `alt?`. Calls `getCredit(creditId)` to resolve the image, renders `<Image priority fetchpriority="high" sizes="100vw">`. Uses `<JsonLd>` for the schema if `schema?` prop passed.
2. **`AttractionGrid.tsx`** — props: `attractions: Array<{slug, name, imageSrc, alt}>`. Uses `<Grid>` primitive.
...
4. **`PhotoGallery.tsx`** — props: `images`, `sizes` (required string). Renders `<Image>` per item with the `sizes` prop. srcset emission is automatic via `next/image` + the deviceSizes config from plan 03.
5. **`StickyCTA.tsx`** — sticky-positioned button using logical `inset-inline-end-4` etc.
6. **`ItineraryCard.tsx`** — props: `days`, `title`, `summary`. Renders `<Card>`-based.
7. **`WhereToStay.tsx`** — props: `priceRange`, `neighborhoods`, `affiliateCards` (consumed in Phase 2; here stubbed).
8. **`TransportInfo.tsx`** — props: `airport`, `transportOptions`, `affiliateCards` (Skyscanner/RentalCars).
9. **`BestTimeToVisit.tsx`** — props: `months`, `seasonHighlights`. Uses Hebrew-aware month names if locale=he.
10. **`ShabbatNotice.tsx`** — STATIC; props: `friStart`, `satEnd`, `notes?`. Renders informational card. NO `'use client'`. NO Hebcal API call. Runtime widget deferred to v2.
11. **`Price.tsx`** — STATIC; props: `ils` (required), `usd?`, `eur?`. Renders all 3 in `<Ltr>` wrappers. NO conversion service. Comment: `// TODO: build-time FX update on Phase 6 cron`.
12. **`AffiliateDisclosure.tsx`** — FTC-compliant inline disclosure. Hebrew: "גילוי נאות: הקישורים שלהלן הם קישורים שותפים. אנו עשויים לקבל עמלה אם תזמינו דרכם, ללא עלות נוספת לכם." English: "Disclosure: The links below are affiliate links. We may earn a commission if you book through them, at no extra cost to you." Inline (not footer-only) per AFF-06.

**Layout components** (in `components/layout/`):
13. **`Header.tsx`** — top nav; uses `<LanguageSwitcher>` + main menu.
14. **`Footer.tsx`** — links to legal pages (Privacy, About, Contact, Affiliate Disclosure, Accessibility Statement); accessibility-statement link uses locale-aware slug (A11Y-05 — actual page exists in Phase 2; here the link generator is locked).
15. **`LanguageSwitcher.tsx`** — uses `next-intl`'s `<Link>` component with `locale` prop; preserves pathname; visually a HE↔EN toggle.
16. **`SkipNav.tsx`** — first child of `<body>`. Reads locale via `useLocale` from `next-intl`; emits Hebrew text on HE pages, English on EN. Targets `#main-content`. CSS: `sr-only focus:not-sr-only` so it's only visible when focused. Per `israeli-accessibility-compliance/SKILL.md`.
17. **`Icon.tsx`** — generic SVG icon wrapper; accepts `directional?: boolean` which adds `rtl:rotate-180` (Tailwind variant).
18. **`Ltr.tsx`** — wraps children in `<bdo dir="ltr">`; use for phone numbers, dates, prices, URLs in HE pages to prevent bidi mishaps.
...
Add `<div id="main-content">` to `app/[locale]/layout.tsx` (after `<SkipNav>`) — already added in plan 01, verify.
...
- `components/__tests__/composites.test.tsx` — smoke tests for all 12 travel composites
- `components/__tests__/skipnav.test.tsx` — A11Y-02 specifics (Hebrew text, first focusable, href target)
- `components/__tests__/form-a11y.test.tsx` — A11Y-06 specifics (uses a sample form; asserts ARIA roles)
- `components/__tests__/photogallery-srcset.test.tsx` — renders, builds, checks built HTML for srcset widths
- `components/__tests__/affiliatecard-stub.test.tsx` — asserts AffiliateDisclosure DOM-precedes the affiliate `<a>` (AFF-06 DOM-order proof)
  </action>
  <verify>
...
- Default state
- Each variant of each variant-having primitive (Button: 3 variants × 3 sizes = 9; Card: 3 variants; Tag: 4 variants; etc.)
...
Create `app/[locale]/admin/components/[component]/page.tsx` — per-component drill-down (e.g., `/admin/components/Button/`). Uses `generateStaticParams` to enumerate every component name.

The `/admin/*` basic-auth middleware is added in plan 10 (audit dashboard). At end of plan 05, the route is reachable without auth — fine; not in robots.txt either way.

After build, the route must work: `pnpm build && pnpm start` then visit `http://localhost:3000/en/admin/components` and `http://localhost:3000/admin/components` (HE root) — both render.
  </action>
  <verify>
    <automated>pnpm build &amp;&amp; (pnpm start &amp; sleep 5 &amp;&amp; curl -s http://localhost:3000/admin/components | grep -c "Button") | grep -v "^0$"</automated>
  </verify>
...
3. **I18N-03**: `pnpm lint components/` exits 0 — zero physical directional utilities in production code.
...
6. **AFF-06 (preparation)**: `<AffiliateCard>` STUB has `<AffiliateDisclosure>` DOM-preceding the link — verified by composedPosition. Production wiring + real-helper integration in plan 06.
